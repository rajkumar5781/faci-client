...
export default {
  ...
  computed: {
    ...
    commissionedTime() {
      let { decommissionLogs } = this
      return dlv(decommissionLogs, '0.commissionedTime', null)
    },
    resourceName() {
      let { rowData } = this
      return dlv(rowData, 'name', null)
    },
    logId() {
      let { rowData } = this
      return dlv(rowData, 'id', null)
    },
    getPortfolioDescription() {
      let { moduleName } = this
      return `${this.$t(
    ...
    portfolioDependency() {
      let { decommissionLogs } = this
      let logValue = JSON.parse(dlv(decommissionLogs, '0.logValue', null))
    ...
    isDecommissioned() {
      let { decommissionLogs } = this
      return dlv(decommissionLogs, '0.decommission', null)
    },
    ...
    getRemarks() {
      let { decommissionLogs } = this
      return dlv(decommissionLogs, '0.remarks', null)
    },
    getCommisionedUser() {
      return dlv(this, 'decommissionLogs.0.commissionedBy.name', '---')
    },
  },
  created() {
