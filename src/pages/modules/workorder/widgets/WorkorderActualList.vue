<template>
  <FContainer height="100%">
    ...
    <FContainer v-else-if="isEmpty(recordList)" height="100%">
      <FContainer display="flex" justifyContent="center" height="100%">
...
export default {
  ...
  computed: {
    optionArray() {
      let actionButtonList = this.widgetDetails?.actionButtonList
      return (actionButtonList || []).map(item => ({
        label: item.label,
  ...
  methods: {
    ...
    //     let height = this.$refs['ac-list'].scrollHeight + 20
    //     let width = this.$refs['ac-list'].scrollWidth
    ...
    btnClickAction(btn) {
      let { record } = btn
      this.handleClickActionBtn(record)
    },
    ...
    clearSelection() {
      ...
      let tableRef = lineItemListRef?.$refs[`list-${this.moduleName}`]
