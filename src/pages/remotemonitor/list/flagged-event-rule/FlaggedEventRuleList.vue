  ...
  methods: {
    changeStatus(record, changedStatus) {
      ...
      this.records = records.map(data => {
        ...
        if (dataId === recordId) {
          let clientId = getProperty(data, 'client.id', 0)
          this.updateStatus(recordId, changedStatus, clientId)
    ...
    async updateStatus(id, status, clientId) {
      let data = { status, id, clientId }
      let { error } = await API.post(
        `/v3/ruleList/flaggedEvent/ruleStatus/${id}`,
        {
          data,
        }
      )
      if (isEmpty(error)) {
        let successMsg = this.$t(
          'flagged_event_rule.status_save_success_message',
          'Status Updated',
          { ns: 'remote_monitor' }
        )
